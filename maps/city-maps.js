// 城邦地图表格生成函数
function displayCityMapsTable(contentBody, quality, color) {
    // 创建容器
    const containerHTML = `
        <div class="city-container">
            <div id="maps-content">
                <div class="map-loading">正在加载地图数据...</div>
            </div>
        </div>
    `;
    
    contentBody.innerHTML = containerHTML;
    loadAndDisplayCityMaps();
}

// 地图按钮数据 - 使用PS中的像素坐标（左上角为原点）
const cityMapButtons = [
    // 地图0 (chapter: 6, mapId: 0)
    {
        mapId: 0,
        type: "portal",
        typeName: "起点",
        name: "",
        description: "进图位置",
        x: 75,
        y: 107
    },
    {
        mapId: 0,
        type: "portal",
        typeName: "传送小站",
        name: "",
        description: "快速传送",
        x: 75,
        y: 83
    },
    {
        mapId: 0,
        type: "portal",
        typeName: "城镇",
        name: "海象栖息地",
        description: "DLC2入口",
        x: 29,
        y: 18
    },
    {
        mapId: 0,
        type: "portal",
        typeName: "城镇",
        name: "光暗城",
        description: "DLC3入口",
        x: 121,
        y: 18
    },
    {
        mapId: 0,
        type: "portal",
        typeName: "特殊",
        name: "神秘之门",
        description: "开启特殊副本",
        x: 75,
        y: 18
    },
    {
        mapId: 0,
        type: "npc",
        typeName: "任务",
        name: "老登",
        description: "领取经验金币",
        x: 57,
        y: 26
    },
    {
        mapId: 0,
        type: "npc",
        typeName: "任务",
        name: "大统领",
        description: "领取经验金币",
        x: 93,
        y: 26
    },
    {
        mapId: 0,
        type: "npc",
        typeName: "特殊",
        name: "世界调谐器",
        description: "调整难度",
        x: 75,
        y: 50
    },
    {
        mapId: 0,
        type: "npc",
        typeName: "任务",
        name: "赏金猎人",
        description: "悬赏任务",
        x: 121,
        y: 42
    },
    {
        mapId: 0,
        type: "npc",
        typeName: "商人",
        name: "军需官",
        description: "贩卖皇家装备",
        x: 121,
        y: 74
    },
    {
        mapId: 0,
        type: "npc",
        typeName: "铁匠",
        name: "军团铁匠",
        description: "升级装备",
        x: 29,
        y: 42
    },
    {
        mapId: 0,
        type: "npc",
        typeName: "铁匠",
        name: "食铁兽",
        description: "吞噬装备",
        x: 29,
        y: 58
    },
    {
        mapId: 0,
        type: "npc",
        typeName: "商人",
        name: "赌徒",
        description: "装备赌博",
        x: 29,
        y: 74
    },
    // 地图1 (chapter: 6, mapId: 1)
    {
        mapId: 1,
        type: "portal",
        typeName: "起点",
        name: "",
        description: "进图位置",
        x: 734,
        y: 449
    },
    {
        mapId: 1,
        type: "portal",
        typeName: "传送小站",
        name: "",
        description: "快速传送",
        x: 72,
        y: 61
    },
    {
        mapId: 1,
        type: "portal",
        typeName: "副本",
        name: "月光城",
        description: "副本规模[8]",
        x: 22,
        y: 18
    },
    {
        mapId: 1,
        type: "portal",
        typeName: "副本",
        name: "地窖",
        description: "副本规模[7]",
        x: 34,
        y: 130
    },
    {
        mapId: 1,
        type: "boss",
        typeName: "祭坛", 
        name: "",
        description: "复活点",
        x: 697,
        y: 449
    },
    {
        mapId: 1,
        type: "boss",
        typeName: "祭坛", 
        name: "",
        description: "复活点",
        x: 403,
        y: 190
    },
    {
        mapId: 1,
        type: "npc",
        typeName: "任务",
        name: "老登镜像",
        description: "领取经验金币",
        x: 53,
        y: 44
    },
    {
        mapId: 1,
        type: "npc",
        typeName: "商人",
        name: "皇家军工团",
        description: "贩卖流星锤/格斗剑",
        x: 99,
        y: 32
    },
    {
        mapId: 1,
        type: "npc",
        typeName: "商人",
        name: "皇家鞋匠",
        description: "贩卖铁板靴/皮靴",
        x: 39,
        y: 84
    },
    {
        mapId: 1,
        type: "npc",
        typeName: "铁匠",
        name: "城邦工匠",
        description: "工匠制作",
        x: 71,
        y: 129
    },
    // 地图2 (chapter: 6, mapId: 2)
    {
        mapId: 2,
        type: "portal",
        typeName: "起点",
        name: "",
        description: "进图位置",
        x: 130,
        y: 675
    },
    {
        mapId: 2,
        type: "portal",
        typeName: "城镇",
        name: "日光城",
        description: "",
        x: 129,
        y: 48
    },
    {
        mapId: 2,
        type: "boss",
        typeName: "祭坛", 
        name: "",
        description: "复活点",
        x: 130,
        y: 650
    },
    {
        mapId: 2,
        type: "boss",
        typeName: "祭坛", 
        name: "",
        description: "复活点",
        x: 130,
        y: 290
    },
    {
        mapId: 2,
        type: "boss",
        typeName: "魔王",
        name: "墨菲特",
        description: "狱火/星落/献祭",
        x: 129,
        y: 80
    },
    // 地图3 (chapter: 6, mapId: 3)
    {
        mapId: 3,
        type: "portal",
        typeName: "起点",
        name: "",
        description: "进图位置",
        x: 21,
        y: 39
    },
    {
        mapId: 3,
        type: "boss",
        typeName: "祭坛", 
        name: "",
        description: "复活点",
        x: 58,
        y: 71
    },
    {
        mapId: 3,
        type: "boss",
        typeName: "祭坛", 
        name: "",
        description: "复活点",
        x: 260,
        y: 388
    },
    {
        mapId: 3,
        type: "boss",
        typeName: "魔王",
        name: "地下梦魇",
        description: "腐蚀喷吐/力量喷吐/缠绕攻击",
        x: 150,
        y: 243
    },
    {
        mapId: 3,
        type: "boss",
        typeName: "首领",
        name: "盗神",
        description: "速度/闪避/命中/暴击++",
        x: 151,
        y: 436
    },
    {
        mapId: 3,
        type: "npc",
        typeName: "神秘商人",
        name: "盗圣",
        description: "贩卖项链/头环",
        x: 33,
        y: 251
    },
    // 地图4 (chapter: 6, mapId: 4)
    {
        mapId: 4,
        type: "portal",
        typeName: "起点",
        name: "",
        description: "进图位置",
        x: 55,
        y: 469
    },
    {
        mapId: 4,
        type: "boss",
        typeName: "祭坛", 
        name: "",
        description: "复活点",
        x: 55,
        y: 453
    },
    {
        mapId: 4,
        type: "boss",
        typeName: "首领",
        name: "上尉幻象",
        description: "召唤海元素",
        x: 37,
        y: 162
    },
    {
        mapId: 4,
        type: "boss",
        typeName: "首领",
        name: "中尉幻象",
        description: "圣光术",
        x: 74,
        y: 162
    },
    {
        mapId: 4,
        type: "boss",
        typeName: "魔王",
        name: "大统领幻象",
        description: "召唤骑士/圣光术/震荡光波",
        x: 55,
        y: 33
    }
];

// 加载并显示地图
function loadAndDisplayCityMaps() {
    const mapsContent = document.getElementById('maps-content');
    
    try {
        const cityMaps = mapManager.getMapsByChapter(6);
        
        if (cityMaps.length === 0) {
            mapsContent.innerHTML = '<div class="map-error">未找到城邦地图数据</div>';
            return;
        }
        
        // 生成地图HTML
        let mapsHTML = '';
        cityMaps.forEach(map => {
            const imagePath = mapManager.getMapImagePath(map.chapter, map.mapId, map.name);
            const mapButtons = cityMapButtons.filter(button => button.mapId === map.mapId);
            
            mapsHTML += `
                <div class="map-item">
                    <h3 class="map-title">${map.name}</h3>
                    <div class="map-image-container" id="map-container-${map.chapter}-${map.mapId}">
                        <img src="${imagePath}" alt="${map.name}" class="map-image" id="map-image-${map.chapter}-${map.mapId}">
                        <div class="map-buttons-container" id="map-buttons-${map.chapter}-${map.mapId}"></div>
                    </div>
                </div>
            `;
        });
        
        mapsContent.innerHTML = mapsHTML;
        cityMaps.forEach(map => addButtonsToCityMap(map.chapter, map.mapId));
        
    } catch (error) {
        console.error('加载地图失败:', error);
        mapsContent.innerHTML = '<div class="map-error">加载地图数据时出错，请刷新页面重试</div>';
    }
}

// 为地图添加按钮（简化版）
function addButtonsToCityMap(chapter, mapId) {
    const buttonsContainer = document.getElementById(`map-buttons-${chapter}-${mapId}`);
    const mapImage = document.getElementById(`map-image-${chapter}-${mapId}`);
    
    if (!buttonsContainer || !mapImage) return;
    
    // 简化容器样式
    buttonsContainer.style.cssText = `
        position: absolute;
        top: 0;
        left: 0;
        width: 100%;
        height: 100%;
        pointer-events: none;
    `;
    document.getElementById(`map-container-${chapter}-${mapId}`).style.position = 'relative';
    
    const mapButtons = cityMapButtons.filter(button => button.mapId === mapId);
    
    // 简化位置计算逻辑
    const updateButtonPositions = () => {
        // 直接使用图片显示尺寸（含边框）
        const displayWidth = mapImage.offsetWidth;
        const displayHeight = mapImage.offsetHeight;
        // 原始图片尺寸
        const originalWidth = mapImage.naturalWidth;
        const originalHeight = mapImage.naturalHeight;
        
        // 计算缩放比例
        const scaleX = displayWidth / originalWidth;
        const scaleY = displayHeight / originalHeight;
        
        mapButtons.forEach(buttonData => {
            const button = mapManager.createMapButton(
                buttonData.type,
                buttonData.typeName,
                buttonData.name,
                buttonData.description
            );
            
            if (button) {
                // 计算调整后坐标
                const adjustedX = buttonData.x * scaleX;
                const adjustedY = buttonData.y * scaleY;
                
                button.style.cssText = `
                    position: absolute;
                    pointer-events: all;
                    z-index: 10;
                    left: ${adjustedX}px;
                    top: ${adjustedY}px;
                    transform: translate(-50%, -50%);
                `;
                
                buttonsContainer.appendChild(button);
            }
        });
    };
    
    // 图片加载完成后计算
    mapImage.onload = updateButtonPositions;
    // 已加载完成则直接更新
    if (mapImage.complete) updateButtonPositions();
}